...
/**
 * @typedef {Object|undefined} IPhaseObj
...
(
    function () {
         ...
         * @returns {ISunCoordinates} // Nothing
         */
         ...
         */
        SunCalc.getSunTimeByAzimuth = function (dateValue, nazimuth, lat, lng, degree) {
            if (isNaN(nazimuth)) {
         ...
         * @param {number} lat latitude for calculating moon-times
         * @param {number} lng longitude for calculating moon-times
         * @returns {{main: (Date|null), invert: (Date|null)}} // Nothing
         */
        SunCalc.moonTransit = function (rise, set, lat, lng) {
            ...
            if (rise) {
                tempTransitAfter = calcMoonTransit(
                    riseValue,
                    SunCalc.getMoonTimes(new Date(riseDate).setDate(day + 1), lat, lng).set.valueOf(),
                );
            ...
            if (set) {
                tempTransitBefore = calcMoonTransit(
                    setValue,
                    SunCalc.getMoonTimes(new Date(setDate).setDate(day - 1), lat, lng).rise.valueOf(),
                );
